...
- **PUT THIS BULLET LAST??** A data frame (`data.frame` or `tibble`), with three or four columns
...
- A matrix (either a base `matrix` or a class from the `Matrix` package), dimension K by K, where K is the number of items. The *i,j*-th element is the number of times item *i* has beaten item *j*.  **MENTION Wij HERE??**
...
**MAYBE SHOW THE GRAPH FOR THAT SECOND ONE??**
...
A `btdata` objects is a list containing two elements: 
